...
<% foreach (var item in ViewData.Model as IEnumerable<image>) 
{
    ...
    <%if (Convert.ToString(ViewData["default_division"]) == "ULS-PA")
      {%>
    <td style="width:50; vertical-align:bottom"><input type="button" onclick="DeleteAssignBeforeImage(<%=item.image_id %>)" value="Delete" style="width:45px; font-size:11px"  /></td>    
    <%} %>
    <%if (alternating) 
    { %>
    </tr>
